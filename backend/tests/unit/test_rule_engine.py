...
# TEST: provisional (pre-2022) -- tax_period < 01/2022, not in GSTR-2B,
# within Rule 36(4) limit -> `provisional`
# TEST: ineligible (post-2022 date gate) -- tax_period >= 01/2022, not in
# GSTR-2B -> `ineligible` (override active, not provisional)
...
from itc.domain.verdict import VerdictType
...
def test_blocked_beats_everything() -> None:
...
def test_time_bar_returns_time_barred_with_correct_reason() -> None:
    facts = make_facts(
...
def test_section_16_2_c_supplier_not_filed() -> None:
...
def test_ineligible_post_2022_date_gate() -> None:
    facts = make_facts(
...
def test_all_conditions_met_returns_eligible() -> None:
